...
#            scale_percent = cv.getTrackbarPos('scale', 'edge')
        ...
        #                print(unscaled)
                        x, y, w, h = unscaled
                        tile = self.get_tile(x, y, w, h, tilesize)
    #                    print(tile, x, y, w, h)
                        #crop
                        cardimage=orig[y: y + h, x : x + w]
                        (tile, card) = self.process_card(cardimage, unscaled, tilesize, page)

                        if (card != None):
                            cv.putText(im2, '%d%d' % (tile[0], tile[1]),
                                       (box[0] + int(box[2] / 2) - 60, box[1] + int(box[3] / 2) + 30),
                                       cv.FONT_HERSHEY_SIMPLEX, 2, (0,0,255), 3)

                        if (False and line != None and conf != None):
...
#        cmd = "scrcpy -s %s --lock-video-orientation=3 --v4l2-sink=/dev/video0 --stay-awake --power-off-on-close --no-display --show-touches" % self.serial
        cmd = "scrcpy -s %s -m1024 --v4l2-sink=/dev/video0 --stay-awake --power-off-on-close --no-display --show-touches" % self.serial
        print(cmd)
