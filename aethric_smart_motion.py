...
BUTTONS = { 'EXIT': 'tap', 'HOLD TO SPAR': 'hold', 'HOLD SPAR': "hold", 'CONTINUE': 'tap'}
...
BTNMAP = {
'Arena': { "summon": 2, "ward": 5, "heal": 4, "mend": 7, "buff": 6, "debuff": 8, "offhand": [20,21] },
...
class Aethric(Actions):
    def __init__(self, *args, **kwargs):
        ...
        self.Arena = ["summon", "summon", "ward", "summon", "ward", "summon", "mend", "summon", "mend", "debuff", "mend", "debuff", "mend"]
...
#                            print("scale: %s cx: %s cy: %s" % (self.scale, cx, cy))
                            ...
                            if (bt == "EXIT"):
                                self.actions.reset()
                            if ((bt == "HOLD SPAR") or (bt == "HOLD TO SPAR")):
                                self.actions.reset()
            ...
            #  decide what to do
            #
            if self.spell_menu and self.image_options in ["Battle", "Arena"]:
                self.turns += 1
                action=""

                if self.mana_percent < 10:
                    action = "potion"

                if self.ward_percent < 10:
                    action = "ward"

                if self.summons < 3 and self.turns < 5:
                    action = "summon"

                if self.ward_percent < 95 and self.summons == 5:
                    action = "mend"

                if self.ward_percent < 25 and self.summons >= 3:
                    action = "ward"

                if self.summons < 5 and self.turns > 8:
                    action = "summon"

                if self.health_percent < 80 and self.summons == 5:
                    action = "heal"

                if self.health_percent < 35 and self.summons >= 2:
                    action = "heal"

                if self.health_percent < 15 and self.summons >= 1:
                    action = "heal"

                if self.health_percent < 9:
                    action = "heal"

                if self.mana_percent < 33:
                    action = "potion"

                if not len(action):
                    action = "offhand"

                feedback = "%s, turn %s" % (action, self.turns);
                cv.putText(im2, feedback, (20, 200), cv.FONT_HERSHEY_SIMPLEX, 1, (0,255,0), 2)
...
#            if (self.image_options
            if (self.image_options in ["Rects", "Arena", "Raid", "Battle"]):
...
##        self.cannyLower = 50
        ...
        options = [QCheckBox("V4L2"), QCheckBox("Battle"), QCheckBox("Arena"), QCheckBox("Raid"), QCheckBox("Rects"), QCheckBox("Thresh"),  QCheckBox("Edge C"), QCheckBox("Sq Edge"), QCheckBox("Sq Edge Contours"), QCheckBox("Motion")]
